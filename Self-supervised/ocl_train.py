# OCL (MiCL) pre-training entry point, reusing VoCo's 3D backbone & data pipeline.
#
# New file: mirrors Self-supervised/voco_train.py but swaps VoCoHead (geometric
# context + EMA teacher) for OCLHead3D (random-mask two-view contrastive).
# The data loader (utils.data_utils.get_loader) and the SwinUNETR backbone are
# reused unchanged. external/OCL is only a read-only reference for the algorithm.
...
def main():
    ...
    def train(args, global_step, train_loader, val_best, scaler):
        ...
        for step, batch in enumerate(train_loader):
            t1 = time()
            # VoCo's loader returns (img, labels, crops); OCL only needs img
            img, _, _ = batch
            img = concat_image(img)
            img = img.cuda()
    ...
    # OCL (MiCL) specific algorithm hyper-parameters
    parser.add_argument("--mask_drop", default=0.3, type=float,
                        help="fraction of 3D blocks randomly dropped to build a view (OCL masking)")
    parser.add_argument("--kappa", default=1.0 / 64.0, type=float,
                        help="OCL tSP softplus temperature (models_mae.compute_tSP)")
    parser.add_argument("--temperature", default=0.07, type=float,
                        help="OCL contrastive temperature (models_mae.compute_tSP)")
